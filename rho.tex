...
\item Step b (determination of $\eta$): fit over range $|t| < 0.0015\un{GeV^2}$, with $b_1$ fixed from step a. The overall $\chi^2$ receives an additional term $(\eta - 1)^2/ \sigma_\eta^2$, $\sigma_\eta = 0.055$, which reflects the constraint from the $\beta^* = 90\un{m}$ data. The fit gives a p-value of $0.11$.
...
\item Step b (determination of $\eta$): fit over range $|t| < 0.0023\un{GeV^2}$, with $b_1$ and product $\eta a^2$ fixed from step a. Since $\eta$ is determined and the product $\eta a^2$ is fixed, $a$ is also determined in this step. The fit yields a p-value of $0.14$.
