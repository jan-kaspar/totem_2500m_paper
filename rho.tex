...
\end{equation}
For our best fit (combination $N_b=3$ with $|t|_{\rm max} = 0.15\un{GeV^2}$) it yields
% TODO
%$\sigma_{\rm tot} = (112.1 \pm 3.0)\un{mb}$.
$\sigma_{\rm tot} = (XXX.X \pm X.X)\un{mb}$.
This value is well compatible with previous publication Ref.~\cite{totem-13tev-90m} even if slightly higher. This order is expected since in the presented analysis the effects of CNI, acting as destructive interference, are subtracted.
