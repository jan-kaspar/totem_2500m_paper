...
\vskip-2mm
\caption{%
Details of fit with $N_b = 3$ and $|t|_{\rm max} = 0.15\un{GeV^2}$. \TODO{add fit parameters}
}
...
\vskip-2mm
\caption{%
Details of fit with $N_b = 1$ and $|t|_{\rm max} = 0.07\un{GeV^2}$. \TODO{add fit parameters}
}
...
\TODO{Why variable normalisation -- QED sensitivity, equation with eta}

The normalisation can be determined from the $\beta^* = 90\un{m}$ data (Section~\ref{sec:normalisation}), from the $\beta^* = 2500\un{m}$ data (this publication) or their combination. This is formalised in the following three approaches.
\begin{itemize}
...
\begin{itemize}
\item step a (determination of $\eta a^2$ and $b_1$): fit over range $0.015 < |t| < 0.05\un{GeV^2}$, CNI effects ignored, therefore the fit is only sensitive to the product $\eta a^2$, \TODO{see Eq ...}
...
\begin{table}
\caption{%
Summary of $\rho$ and total cross-section results \TODO{round numbers}.
}%
\vskip-5mm
