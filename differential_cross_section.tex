...
\caption{%
Differential cross-section from Table \ref{tab:data} with statistical (bars) and systematic uncertainties (bands). The yellow band represents all systematic uncertainties, the green one all but normalisation. The bands are centred around a data fit including both nuclear and Coulomb components (the fit shown in Figure~\ref{fig:fit exp1 0.07}). INSET: a low-$|t|$ zoom of cross-section rise due to the Coulomb interaction. \TODO{plot uncertainty bars around points instead of fit ??, plot exactly what is in the table ??}
}
\label{fig:dsdt}
\end{center}
\end{figure*}
